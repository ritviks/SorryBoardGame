...
public class GameManager {
	...
	//info for jump priority
	private boolean passedOrJumpedOpponent=false;
	private int botMoveSeven=-1;
	...
	private boolean mustGoBack=false;
	//To get list of ValidPawns
	private Vector<Pawn>validPawns;

	//GamePanel to update
	private GamePanel mGamePanel;
	private Pawn[] validMovablePawns;
	//Load in the players, board, and card deck used for play
	{	validPawns=new Vector<Pawn>();
	...
	public void drawCard() {
		...
		//get list of cli
		CardDialog temp=new CardDialog(card.getName());
	...
	private void validateTravel() {
		...
		Pawn[] temp=livePlayers[currentPlaying].getPawns();
		for(int i=0; i<temp.length;i++){
//			temp[i].getCurrentTile().setHover(true);
			Vector<Tile> t =fakeTileClicked(temp[i].getCurrentTile(),livePlayers[currentPlaying]);

			temp[i].getCurrentTile().results=t;
//			if(t!=null){

				temp[i].getCurrentTile().setHover(true);
				if(t!=null)
					for(int k=0;k<t.size();k++){
					t.get(k).showWhenClicked=true;
				}
//			}
		}
	...
	private Vector<Tile> fakeTileClicked(Tile currentTile, Player player) {
		...
		if(card!=null){
			if(card.getType() == CardDeck.SORRY) {
				if(currentTile.isStart()){
					for(int i=0;i<livePlayers.length;i++){
						if(i!=mainPlayer){
							Pawn[] pList=livePlayers[i].getPawns();
							for(int k=0;k<pList.length;k++){
								if(!pList[k].getCurrentTile().isSafeZone()
										||!pList[k].getCurrentTile().isStart()
										||!pList[k].getCurrentTile().isHome()){
									result.add(pList[k].getCurrentTile());
								}
							}
						}
					}
			...
			int moves=CardDeck.getValue(card);
			Tile endTile=preCheckMove(currentTile,moves);
			if(endTile!=null) 
			{
				result.add(endTile);
				return result;
	...
	private void endTurn() {
		...
		for(int i=0; i<temp.length;i++){
			...
			//			if(fakeTileClicked(ghostPawn.getCurrentTile(),livePlayers[currentPlaying]));
			temp[i].getCurrentTile().setHover(false);
			mGamePanel.mustRemoveHowers=true;
		}
	...
	public void calculateScore() {
		...
		Player currPlayer=getMainPlayer();
		Pawn[] playersPawns=currPlayer.getPawns();
		for(int i=0;i<playersPawns.length;i++){
			if(playersPawns[i].getCurrentTile().isHome())
				score+=5;
		}
		for(int i=0; i<livePlayers.length;i++){

			if(livePlayers[i]!=getMainPlayer()){

				Pawn[] opponentsPawns=players[i].getPawns();
				for(int k=0; k<opponentsPawns.length;k++){
					if(!opponentsPawns[k].getCurrentTile().isHome())
						score+=3;
					if(opponentsPawns[k].getCurrentTile().isStart())
						score+=1;
				}
			}
		}
	...
	private void simulatePlayer(Player player) {
		...
		if(card!=null){
			boolean done=false;
			if(card!=null){
				if(card.getType()==CardDeck.ONE||card.getType()==CardDeck.TWO||card.getType()==CardDeck.SORRY){
					for(Pawn pawn : player.getPawns()) {
						if(pawn.getCurrentTile().isStart())
						{
							if(tileClicked(pawn.getCurrentTile(),player)) {
								done=true;
								break;}
						}
					}
			...
			if(!done){//if can jump pawn
				for(Pawn pawn : player.getPawns()) {
					if(card!=null){
						if(card.getType()!=CardDeck.ELEVEN){

							if(!pawn.getCurrentTile().isHome()&&!pawn.getCurrentTile().isStart()){

								///different for card 7
								if(card!=null){
									if(card.getType()==CardDeck.SEVEN){
										for( int i=1; i<=7; i++){
											Tile tempTile=travel(pawn.getCurrentTile(),pawn,i);
											if(tempTile!=null){
												tempTile=slide(tempTile,pawn);
												if(passedOrJumpedOpponent){
													botMoveSeven=i;
													if(tileClicked(pawn.getCurrentTile(),player)){ 
														done=true;
														passedOrJumpedOpponent=false;
														break;
													}
												}
											}
								...
								//different for card 10
								if (card!=null){
									if(card.getType()==CardDeck.TEN){
										Tile tempTile=travel(pawn.getCurrentTile(),pawn,10);
										if(tempTile!=null){
											tempTile=slide(tempTile,pawn);
											if(passedOrJumpedOpponent){
												if(tileClicked(pawn.getCurrentTile(),player)){ 
											...
											tempTile=travel(pawn.getCurrentTile(),pawn,-1);
											if(tempTile!=null){
									...
									if(card!=null){
										Tile tempTile=travel(pawn.getCurrentTile(),pawn,CardDeck.getValue(card));
										if(tempTile!=null){
											tempTile=slide(tempTile,pawn);
											if(passedOrJumpedOpponent){
			...
			if(!done){
				//get farthest forward	

				if(card!=null){
					card.getType();
					if(card.getType()!=CardDeck.FOUR){
						Tile start=player.getPawns()[0].getStartTile();
						int counter=0;
						while(counter!=61){
							if(tileClicked(start,player)){ 
								done=true;
								break;
							}
			...
			if(!done){	// least backward
				...
				while(!start.isHome()){
					...
					start=start.getNext(player.getColor());
				}
			}
	...
	public boolean tileClicked(Tile tile, Player player) {
		...
		if(card.getType() == CardDeck.SEVEN && sevenRemainder != -1) {
			...
			if(options == 0) return false;
				...
				} else n = botMoveSeven;// Integer.valueOf(availableMoves[0]); //Robot will just use first valid move
			...
			if(currentPlaying != mainPlayer) {
				...
				if(canMoveForward&&(mustGoBack==false)) {
	...
	public Tile travel(Tile start, Pawn pawn, int numSpaces) {
		...
		for(int i = 0; i <numSpaces; i++) {
			...
			if(start.isOccupied())passedOrJumpedOpponent=true;
		}
		//		int newDist=pawn.getDistance()+dist;
	...
	public Tile slide(Tile temp, Pawn pawn) {
		if(temp.doesSlide() && !temp.getPrevious().doesSlide()) {
			while(temp.getNext(pawn.getColor()).doesSlide()) {
				...
				if(temp.isOccupied())passedOrJumpedOpponent=true;
			}
			temp = temp.getNext(pawn.getColor());
			if(temp.isOccupied()){
				if(temp.getPawnColor()!=pawn.getColor()) passedOrJumpedOpponent=true;
			}
		}
		return temp;
	}
	...
	public Tile preCheckMove(Tile tile, int moves) {

		Tile temp=tile;
		boolean backwards = moves < 0;
		...
		for(int i = 0; i <moves; i++) {
			...
			if(backwards){ 
				...
				if(tile.getPawn()!=null)
					if(tile.getPawn().getColor()!=null)
						temp = temp.getNext(tile.getPawn().getColor());

			}
			if(temp.isOccupiedByColor(tile.getPawn().getColor())) return null;
			if(temp.isOccupied())passedOrJumpedOpponent=true;
		}
